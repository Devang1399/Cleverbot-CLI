...
class Listener():
    ...
    def _get_google_transciption(self):
        ...
        google_speech_api_url = 'https://www.google.com/speech-api/v2/recognize?output=json&xjerr=1&client=chromium&key=AIzaSyBOti4mM-6x9WDnZIjIeyEU21OpBXqWBgw&pfilter=2&lang=%s&maxresults=%s'%(self.language, self.max_results)
        ...
        # fix JSON compatability
        true = True
        JSON = url_handle.read()
        JSON = JSON.split('\n')

        result = eval(JSON[1]) if JSON[1] != '' else None
